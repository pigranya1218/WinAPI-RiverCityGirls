...
void RestrictMoveRect::checkCollision(Vector3 * poses, Vector3 size)
{
		...
		_point[3].x < checkMid[0].x && checkMid[0].x < _point[2].x) // 윗 중점이 평행사변형 아랫변에 부딪혔을때
	{
		...
		Vector3 newPos = Vector3(checkMid[0].x, checkMid[0].y, checkMid[0].z + height);
		for (int k = 0; k < 4; k++)
		{
			poses[k] = Vector3(newPos.x + dir[k][0] * width, newPos.y, newPos.z + dir[k][1] * height);
		}
	}
		...
		_point[0].x < checkMid[1].x && checkMid[1].x < _point[1].x) // 아랫 중점이 평행사변형 윗변에 부딪혔을때
	{
		...
		Vector3 newPos = Vector3(checkMid[1].x, checkMid[1].y, checkMid[1].z - height);
		for (int k = 0; k < 4; k++)
		{
			poses[k] = Vector3(newPos.x + dir[k][0] * width, newPos.y, newPos.z + dir[k][1] * height);
		}
	...
	for (int i = 0; i < 4; i++) 
	{
			...
			(_lines[3]->getValueType(checkPos.x, checkPos.z) == LINEAR_VALUE_TYPE::UP))
		{
			...
			// 바뀐 좌표를 기준으로 다시 그린다
			Vector3 newPos = Vector3(minX - dir[i][0] * width, checkPos.y, minZ - dir[i][1] * height);
			for (int k = 0; k < 4; k++)
			{
				poses[k] = Vector3(newPos.x + dir[k][0] * width, newPos.y, newPos.z + dir[k][1] * height);
			}
		...
		poses[0].z < _point[1].y && _point[1].y < poses[1].z)
	{
		...
		// 바뀐 좌표를 기준으로 다시 그린다
		Vector3 newPos = Vector3(poses[0].x - dir[0][0] * width, poses[0].y, poses[0].z - dir[0][1] * height);
		for (int k = 0; k < 4; k++)
		{
			poses[k] = Vector3(newPos.x + dir[k][0] * width, newPos.y, newPos.z + dir[k][1] * height);
		}
		...
		poses[0].z < _point[3].y && _point[3].y < poses[1].z)
	{
		...
		// 바뀐 좌표를 기준으로 다시 그린다
		Vector3 newPos = Vector3(poses[2].x - dir[2][0] * width, poses[2].y, poses[2].z - dir[2][1] * height);
		for (int k = 0; k < 4; k++)
		{
			poses[k] = Vector3(newPos.x + dir[k][0] * width, newPos.y, newPos.z + dir[k][1] * height);
		}
